...
class Main extends React.Component {
  constructor() {
    ...
    this.dataAndMaps = {
      ...
      'dual swords': [dsData, dsMap, 'ds']
    };
    this.weaponAlts = {
      'sword and shield': ['Dual Swords', 'ds'],
      'dual swords': ['Sword and Shield', 'sns']
    };
    this.abbr = {
      'sword and shield': ['sns', 'ds'],
      'dual swords': ['ds', 'sns']
    };
  ...
  componentDidUpdate() {
    ...
    if (this.state.mode === "tree") {
      if (document.querySelector('.active-border')) document.querySelectorAll('.active-border').forEach(border => border.classList.remove('active-border'));
      ...
      if (this.state.itemSelect && this.state.itemSelect.type == this.dataAndMaps[this.state.subTitle][2]) {
        ...
        if (this.state.itemSelect && this.state.itemSelectBorder) document.getElementById(this.state.itemSelectBorder).classList.add('active-border');else {
  ...
  displayList() {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      class: "list-header"
    }, "Slots"), /*#__PURE__*/React.createElement("td", {
      class: "list-header"
    }, "Defense"), /*#__PURE__*/React.createElement("td", {
      class: "list-header"
    }, "Rarity"), /*#__PURE__*/React.createElement("td", {
      class: "list-header"
    }, "Tree Link"))), /*#__PURE__*/React.createElement("tbody", null, theList.map(weapon => {
      return /*#__PURE__*/React.createElement("tr", {
        id: `${weapon.name.toLowerCase()}-row`,
        class: weapon.name === this.state.itemSelect.name ? "header-bold" : null,
        onClick: this.clickHandler
      }, /*#__PURE__*/React.createElement("td", {
        id: `${weapon.name.toLowerCase()}-name`,
        class: "list-cell pr-2"
      }, weapon.name), /*#__PURE__*/React.createElement("td", {
        ...
        id: `${weapon.name.toLowerCase()}-slots`,
        class: "list-cell pr-2"
      }, weapon.slots), /*#__PURE__*/React.createElement("td", {
        id: `${weapon.name.toLowerCase()}-defense`,
        class: "list-cell pr-2"
      }, weapon.bonus), /*#__PURE__*/React.createElement("td", {
        id: `${weapon.name.toLowerCase()}-rarity`,
        class: "list-cell pr-2"
      }, weapon.rarity), /*#__PURE__*/React.createElement("td", {
        id: `${weapon.name.toLowerCase()}-tree-link`,
        class: "list-cell text-center",
        onClick: this.clickHandler
      }, weapon.name.endsWith("G") || weapon["upgrade-to"] === "N/A" && weapon["upgrade-from"] === "N/A" ? null : /*#__PURE__*/React.createElement("span", null, "O")));
    }))));
  }
  ...
  displayTree() {
    ...
    const currData = this.dataAndMaps[this.state.subTitle][0]; // modify grid based on currently collapsed trees
    ...
    newGrid.forEach((row, ind) => {
      if (collapsed.includes(row[0])) countFlag = true; // this is a tree that is collapsed, flag it 
              ...
              // this is a row that comes after a potentially flagged tree-row
              if (countFlag) {
    ...
    newGrid.forEach((row, y) => {
      ...
      row.forEach((cell, x) => {
        ...
        if (cell !== "" && cell !== "+") {
          ...
          let sub = this.state.subTitle; // weapon icon

          theTree.push( /*#__PURE__*/React.createElement("image", {
            id: `${cell.includes(this.weaponAlts[sub][1]) ? this.weaponAlts[sub][1] : this.dataAndMaps[sub][2]}icon-${itemMod}`,
            class: "main-icon",
            "data-weapon": `${cell}`,
            href: cell.includes(this.weaponAlts[sub][1]) ? `./public/icons/${this.weaponAlts[sub][1]}.png` : `./public/icons/${this.dataAndMaps[sub][2]}.png`,
            x: paddingX + boxSpaceX * x,
            y: paddingY + boxSpaceY * y,
            width: boxWidth,
            height: boxWidth,
  ...
  clickHandler(event) {
    event.preventDefault(); // handle click on zoom buttons

    if (event.target.id.startsWith("zoom")) this.btnTreeZoom(event.target.id); // handle click on mode button
    else if (event.target.id.startsWith("mode")) {
        this.setState({
          "mode": this.state.mode === "tree" ? "list" : "tree"
        });
  ...
  btnListTableHeader(textContent) {
    ...
    if (!['Sharpness', 'Tree Link', ''].includes(textContent)) {
      ...
      if (sortKey == this.state.listSortBy) {
        this.setState({
          listSortOrder: this.state.listSortOrder === 'asc' ? 'dec' : 'asc'
        });
      } else {
  ...
  btnTreeAltIcon(weaponName, iconPos) {
    this.setState({
      ...
      if (document.querySelector('.active-border')) document.querySelector('.active-border').classList.remove('active-border'); // find the new border
      ...
      if (iconPos.startsWith('x0')) {
        // the icon was the start of the other tree's branch
        console.log(iconPos);
        let borders = Array.from(document.querySelectorAll('.icon-border'));
        ...
        if (weapon["upgrade-from"].length != 2) {
          // if not equal to two, it's not an array.
  ...
  tooltip() {
    ...
    return /*#__PURE__*/React.createElement("table", {
      ...
      id: "tool-name",
      colspan: "2"
    }, !hoverItem ? "*" : hoverItem.name + (altType ? ` (${this.weaponAlts[this.state.subTitle][0]})` : "")), /*#__PURE__*/React.createElement("th", {
      id: "compare-name",
      colspan: "1"
    }, compare ? selected.name : null))), /*#__PURE__*/React.createElement("tbody", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("b", null, !hoverItem || altType ? "Click to change trees" : "Attack")), /*#__PURE__*/React.createElement("td", {
      id: "tool-attack"
    }, !hoverItem || altType ? '' : hoverItem.attack), /*#__PURE__*/React.createElement("td", {
      id: "compare-attack",
      class: "pl-2"
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      id: "main-app-row",
      class: "row m-0"
    }, this.state.mode === "tree" ? /*#__PURE__*/React.createElement("div", {
      id: "zoomInBtn",
      onClick: this.clickHandler
    }, "+") : null, this.state.mode === "tree" ? /*#__PURE__*/React.createElement("div", {
      id: "zoomOutBtn",
      onClick: this.clickHandler
    ...
    }, this.state.mode === 'tree' ? "List" : "Tree"), this.state.mode === "tree" ? /*#__PURE__*/React.createElement("div", {
    ...
    }, this.displayTree()) : this.displayList(), /*#__PURE__*/React.createElement("div", {
      id: "info-panel",
      ...
      class: "text-center",
      colspan: "2"
    }, "Upgrade"))), /*#__PURE__*/React.createElement("tbody", null, Object.keys(selectedWeapon).filter(val => val.startsWith('upgrade')).map(key => {
      let title = key.split('');
      title[0] = title[0].toUpperCase();
      title = title.join('').split('-').join(' ');
      return /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
